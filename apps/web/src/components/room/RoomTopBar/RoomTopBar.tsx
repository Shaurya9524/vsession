...
import { EndSessionConfirm } from "../EndSessionConfirm/EndSessionConfirm"
...
export function RoomTopBar({ isHost, members, onToggleMembers }: RoomTopBarProps) {
  const { roomId, leaveRoom, endSession } = useRoom()
    ...
    <>
      <header className={styles.topBar}>
        ...
        <div className={styles.right}>
          ...
          <button className={styles.btnGhost} onClick={leaveRoom}>Leave Room</button>
          {isHost && <button className={styles.btnDanger} onClick={() => setConfirmingEnd(true)}>End Session</button>}
        </div>
      </header>

      {confirmingEnd && <EndSessionConfirm onConfirm={endSession} onCancel={() => setConfirmingEnd(false)} />}
    </>
  )
}
